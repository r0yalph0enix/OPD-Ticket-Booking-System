...
if (isset($_POST['submit'])) {
  ...
  $valid = "select * from patient where id = '$id' AND username = '$username'  AND email = '$email' AND password = '$password' AND phone = '$phone' AND address = '$address' AND dateOfBirth = '$dateOfBirth' ";
...
?>
// $selectquery = "INSERT INTO `appointment`('dname','d_id','specialist','price','p_id', 'pname','pcontact','reason_for_appointment','email','address','appointmentdate','appointmenttime')
...
//  VALUES('$dname','$d_id','$specialist','$price','$pid','$pname','$pcontact','$preason','$email','$address','$appointmentdate','$appointmenttime')";


// if ($con->query($selectquery) === TRUE)
if ($con->query($selectquery) === TRUE){
  echo "<script>alert('Congratulation, Appointment has been booked !!');</script>";
  echo "<script>location.replace('patient_doctor_schedule.php');</script>";
} else {
  echo "<script>alert('Opps!! Appointment not booked')<script>";
}
